...
- `app.json`'s `ios.buildNumber` is NOT the source of truth Xcode builds from — the checked-in `ios/ClearHiiT/Info.plist` has a hardcoded `CFBundleVersion` string that only syncs from `app.json` via `expo prebuild`, which this project doesn't run. When bumping the build number, update both `app.json` and `ios/ClearHiiT/Info.plist` (`CFBundleVersion`) or Xcode will keep building the stale number.
